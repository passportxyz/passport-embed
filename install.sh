...
if [ ! -f dev/.env ]; then
    echo ""
    echo "📝 Creating dev/.env file from .env.mock template..."
    cp dev/.env.mock dev/.env
    echo "✅ Created dev/.env file (copied from .env.mock)"
    echo "   Edit dev/.env if you want to use real API instead of mocks"
else
    echo "✅ dev/.env file already exists"
fi
