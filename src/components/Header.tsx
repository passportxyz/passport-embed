...
const ScoreIndicator = ({ score, threshold }: { score: number; threshold: number }) => {
  ...
  const gradientStyle = {
    background: `conic-gradient(
      ...
      color-mix(in srgb, rgb(var(--color-white-c6dbf459)) 94%, rgb(var(--color-primary-c6dbf459)) 6%) ${angle}deg,
      color-mix(in srgb, rgb(var(--color-white-c6dbf459)) 94%, rgb(var(--color-primary-c6dbf459)) 6%) 360deg
    )`,
  };
  ...
  return (
    ...
    >
      <path
        ...
        fill="rgba(var(--color-secondary-c6dbf459), 1)"
      />
    </svg>
  );
};
